...
public class Car {
    ...
    private boolean isOn; //초기는 시동이 꺼진 false 상태

    private int speed; //현재시속

    /* 시동이 걸려있는 상태인 경우에는 할 일이 없고, 시동이 걸려있지 않은 상태인 경우 시동을건다.*/
    ...
    public void startUp() {
        if (isOn) {
            ...
            this.isOn = true;
            System.out.println("시동을 걸었습니다. 출발준비 완료 되었습니다. 부릉");
        }
    }

    /*시동이 걸린 상태인 경우에만 호출시마다 10km/h씩 속도를 증가시킨다.
     * 시동이 걸려있지 않으면 시동을 먼저 걸어주세요 라고 출력
     * */
    public void go() {
        ...
        if (isOn) {
            ...
            this.speed += 10;
            System.out.println("현재의 차의 시속은 " + this.speed + "km/h 입니다.");
        } else {
            System.out.println("차의 시동이 걸려있지 않습니다. 시동을 먼저 걸어주세요.");
        }
    }

    /*시동이 걸려있고 달리는 상태인 경우에만멈출 수 있다.
    ...
    public void turnOff() {
        if (isOn) {
            if (speed > 0) {
                ...
                this.isOn = false;
                System.out.println("시동을 끕니다. 다시운행하시려면 시동을 켜주세요.");
            }
        } else {
            System.out.println("이미 시동이 꺼져있는 상태입니다. 시동상태를 확인해주세여.");
        }
    }
}
