...
public class BiDirectionTests {
    ...
    *  객체는 서로 다른 다 단방향 참조를 합쳐서 양방향이라고 한다.
    * 따라서 두 개의 연관 관계 중 연관 관계의 주인을 정하고,
    * 주인이 아닌 연관관계를 하나 더 추가하는 방식으로 작성하게 된다.
    * 양방향 연관 관계는 항상 설정하는 것이 아니라 반대 방향으로도 접근하여
    *  객체 그래프탐색을 할 일이 많은 경우에만 사용한다.
    *
    ...
    * 연관 관계의 주인은 외래키를 가지고 있는 엔티티이다.*/
    ...
    public void bidirectionAssociationTest(){
        ...
        int categoryCode = 3;
        //when
        /*진짜 연관 관계 : 처음 조회 시부터 조인한 결과를 인출해온다.*/
        Menu foundMenu = entityManager.find(Menu.class, menuCode);
        //가짜 연관 관계 : 해당 엔티티를 조회하고 필요 시 연관 관계 엔티티르 조회하는 쿼리를 다시 실행하게 된다.
        ...
        /*
        * 주의사항!
        * toString() 오버라이딩 시 양방향 연관 관계는 재귀호출이 일어나기 때문에 stackOverFlowError가 발생
        * 따라서 재귀가 일어나지 않게 하기 위해서는 엔티티의 주인이 아닌 쪽의 toString을 연관객체 부분이 출력되지 않도록 해야함.
        * 특히 자동 완성 및 롬복 라이브러리를 이용하는 경우 해당 문제 발생 가능성이 매우 높아진다.
        * */
        System.out.println("foundMenu = " + foundMenu); //영속화되어있는 것을 출력
        System.out.println("foundCategory = " + foundCategory); //영속화되어있는 것을 출력

        foundCategory.getMenuList().forEach(System.out::println); //
    }

    /*
    * 연관 관게의 주인에는 값을 입력하고, 주인이 아닌 곳에는 값을 입력하지 않을 때,
    ...
    * */
    ...
    public void bidirectionAssociationInsertTest(){
        ...
        //given
        Menu menu = new Menu();
        menu.setMenuCode(501);
        menu.setMenuName("연관관계주인메뉴1");
        menu.setMenuPrice(10000);
        menu.setOrderableStatus("Y");

        menu.setCategory(entityManager.find(Category.class, 4));//select
        ...
        entityTransaction.commit(); //insert 전송
    ...
    @Test
    public void bidirectionAssociationNotOwnerTest(){
        //given
