...
public class PrimaryKeyMappingTests {
    ...
    /*Primary key 에는 @Id 어노테이션과 @GeneratedValue 어노테이션을 사용한다
      ...
      @GeneratedValue 어노테이션은 다음과 같은 속성을 가진다.
     */
    /*
    * - strategy : 자동 생성 전략을 지정
   - GenerationType.IDENTITY : 기본 키 생성을 데이터베이스에 위임 // (MySQL의 AUTO_INCREMENT) 안넣으면 동작 안함
   - GenerationType.SEQUENCE : 데이터베이스 시퀀스 객체 사용(ORACLE의 SEQUENCE) // 오라클버전 오토인크리먼트
   - GenerationType.TABLE : 키 생성 테이블 사용 //시퀀스라던지 오토인크리먼트가 없는 테이블
   - GenerationType.AUTO : 자동 선택 (MySQL이라면 IDENTITY, ORACLE이라면 SEQUENCE로 선택) // 알아서 인크리먼트, 시퀀스 적용
