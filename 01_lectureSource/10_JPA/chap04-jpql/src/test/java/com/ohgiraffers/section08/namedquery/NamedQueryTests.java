...
public class NamedQueryTests {
    ...
    /*동적쿼리 : 현재 우리가 사용하는 방식처럼 EntityManager 가 제공하는 메소드를 이용하여
                JPQL을 문자열로 런타임 시점에 동적으로 쿼리를 만드는 방식
                (동적으로 만들어질 쿼리를 위한 조건식이나 반복문은 자바 코드를 이용할 수 있따.)
    * 정적쿼리 : 미리 쿼리를 정의하고 변경하지 않고 사용하는 쿼리를 말하며
    *            미리 정한 코드는 이름을 부여해서 사용하게 된다. 이를 NamedQuery라고 한다.*/
