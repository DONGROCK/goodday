...
Symbol은 ES6에서 도입되 7번째 데이터 타입으로 변경 불가능한 원시 타입의 값이다.
symbol은 다른 값과 중복 되지 않는 유일무이한 값으로 주로 이름 충독의 위험이 없는 유일한 프로퍼티 키를 만들기 위해 사용된다.
*/
...
//반대로 Symbol.keyFor(symbol)를 사용하면 이름을 얻을 수 있따.
...
/*
    심볼은 이름이 같더라도 값이 항상 다르므로 이름이 같을 때 값도 같길 원한다면 전역 레지스트리를 사용해야한다.
    전역 심볼레지스트리는 애플리케이션 곳곳에서 심볼 이름을 이용해 특정 프로퍼티에 접근해야 할 경우 사용할 수 있다.
*/
