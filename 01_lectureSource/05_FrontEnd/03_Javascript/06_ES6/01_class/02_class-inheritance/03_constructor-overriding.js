    //constructor overriding (생성자 오버라이딩)
    //클래스가 다른 클래스를 상속받고 constructor가 없는 경우에는 비어있는 constrcutor가 만들어진다.
...
*/
    ...
    class Deer extends Animal {
        ...
        hide(place){
            console.log(`${this.name}은(는) ${place}에 들어갔습니다.`);
            // console.log(`${this.weight}kg ${this.name}(은)는${this.legLength}번 ${place}에 숨습니다.`);
            // console.log('====================');
        ...
        let deer = new Deer('황정민 사슴', 130, 1);
        deer.hide('엘리베이터 안');
         ...
         * 상속클래스의 생성자 함수가 실행되면 빈객체를 만들고 this에 이 객체를 할당하는 일을 부모 클래스의 생성자가 처리해주길 기대한다.
         * 이런 차이 때문에 상속 클래스의 생성자에선 super를 호출해 부모 생성자를 실행해 주어야 하고 그렇지 않으면
         * this가 될 객체가 만들어지지 않아 에리가 발생한다.
         * 
         */