...
public class FirstController {
     ...
     *
     * 파라미터 선언부에 WebRequest 타입을 선언하면 해당 메소드 호출 시 인자로 값을 전달 해준다.
     * 핸들러 메소드 매개변수로 HttpServletRequest, HttpServletResponse도 사용 가능하다.
     * WebRequest는 HttpServletRequest의 요청 정보를 거의 대부분 그대로 가지고 있는 API로
     * Servlet에 종속적이지 않다.
    ...
    /*
    * 2. @RequestParam로 요청 파라미터 전달받기
    *   요청 파라미터를 매핑하여 호출 시 값을 넣어주는 어노테이션으로 매개변수 앞에 작성한다.
    *   form의 name속성값과 매개변수의 이름이 다른 경우 @RequestParam("name")을 설정하면 된다.
    *   또한  어노테이션은 생략가능하지만 명시적으로 작성하는 것이 의미 파악에 쉽다.
    *
    *   전달하는 form의 name 속성이 일치하는 것이 없는 경우 400에러가 발생하는데 이는 required속성의 기본 값이
    *   true이기 때문이다. required속성을 false로 하게되면 해당 name값이 존재하지 않아도 null로 처리하며 에러가 발생하지 않는다.
    *
    *
    *
    *   값이 넘어오기 않게 되면 "" 빈 문자열이 넘어오게 되는데, 이 떄 parsing관련 에러가 발생할 수 있다.
    *   값이 넘어오지 않는 경우 defaultValue를 이용하게 되면 기본값으로 사용할 수 있따.
    * */
    @PostMapping("modify")
    public String modifyMenuPrice(Model model, @RequestParam(required = false) String modifyName,
        ...
        String message = modifyName + "메뉴의 가격을 " + modifyPrice + "원으로 변경하였씁니다.";
    ...
    /*파라미터가 여러 개인 경우 맵으로 한번에 처리 할 수도 있따.
    * 이 떄 밉의 키는 form의 name 속성값이 된다.
    * */
    ...
    public String modifyMenu(Model model, @RequestParam Map<String, String> parameters){
        ...
        String message = "메뉴이 이름을 " + modifyMenu + "으로, 가격을 " + modifyPrice + "원으로 변경하였습니다.";
    ...
    *
    * 경우에 따라 폼에서 입력한 값을 다음 화면으로 바로 전달해야 하는 경우 발생하는데 이 때 유용하게 사용할 수 있다.
    ...
    * */
    ...
    클래스 레벨에 @SessionAttributes어노테이션을 이용하여 세션값에 값을 담을 key값을 설정해두면
    Model영역에 해당 key로 값이 추가되는 경우 session에 자동등록한다.
    */
    ...
    /*@SessionAtrributes로 등록한 값은 session의 상태를 관리하는 SessionStatus의 setComplete()메소드를 호출해야
    * 사용이 만료된다.*/
    ...
    * 해당 어노테이션은 http본문(body)자체를 읽는 부분을 모델로 변환시켜 주는 어노테이션이다.
    *
    * 출력해보면 쿼리스트링 형태의 문자열이 전송된다.
    * JSON으로 전달되는 경우 jackson의 컨버터로 자동 파싱하여 사용할 수 있따.
    *
    * 주로RestApi작성 시 많이 사용되며, 일반적인 form 전송을 할 때는 거의 사용하지 않는다.
    *
    * 추가적으로 헤더에 대한 정보도
    ...
    * */
    ...
    public void bodyTest(@RequestBody String body,
                         @RequestHeader("content-type") String contextType,
