...
public class FirstController {
    ...
    /*
    * 컨트롤러 핸들러 메서드의 반환 값을 void로 설정하면 요청 주소가 view의 이름이된다.
    ...
    *  true이기 때문이다. required속성을 fasle로 하게되면 해당 name값이 존재하지 않아도 null로 처리하며 에러가 발생하지 않는다.
    *
    *  값이 넘어오지 않게 되면 "" 빈 문자열이 넘어오게 되는데, 이 때 parsing관련 에러가 발생할 수 있다.
    ...
    * */
    ...
    /*
    * 파라미터가 여러 개인 경우 맵으로 한번에 처리할 수 도 있다.
    ...
    * 클래스레벨에 @SessionAttributes어노테이션을 이용하여 세션에 값을 담을 key값을 설정해두면
    ...
    * */
    ...
    /* @SessionAttributes로 등록한 값은 session의 상태를 관리하는 SessionStatus의 setComplete()메소드를 호출해야
    * 사용이 만료된다.
    * */
    @GetMapping("logout2")
    ...
    *
    * 출력해보면 쿼리스트링형태의 문자열이 전송된다.
    * JSON으로 전달되는 경우 Jackson의 컨버터로 자동 파싱하여 사용할 수 있다.
    *
